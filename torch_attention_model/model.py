...
class Network(nn.Module):
    ...
    def fit(self, x, y, x_test, y_test, epochs,
        ...
        with torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(Config['output_model']+'log'),
            with_stack=True
            ) as profiler:
            ...
            while epoch < epochs:
                ...
                for x_batch, y_batch in trainloader:
                    ...
                    # print(x_batch.shape, tau.shape)
                    ##########################################
                    y_batch = y_batch.float()+torch.normal(0,1e-10, y_batch.shape)
                    ...
                    current_loss_R  += (1/batches) * (los_R.cpu().item() - current_loss_R)
                    current_loss_E  += (1/batches) * (los_E.cpu().item() - current_loss_E)
                    
                    current_loss_uq_R += (1/batches) * \
                        (los_uq_R.cpu().item() - current_loss_uq_R)
                    current_loss_uq_E += (1/batches) * \
                        (los_uq_E.cpu().item() - current_loss_uq_E)
