...
class Network_inverter(nn.Module):
    def __init__(self, input_shape=151, k=20):
        ...
        self.l1=nn.Linear(in_features=151, out_features=200)
        self.l2=nn.Linear(in_features=200, out_features=200)
        self.n_peaks = k
    ...
    def forward(self, x):
        ...
        alpha_out_list= []
        # for i in range(self.n_peaks):
        #     alpha_out_list.append(torch.sigmoid(self.alpha_internal_layers[i](out)))
...
class Network_selector(nn.Module):
    def __init__(self, input_shape=151, k=20):
        ...
        self.l2=nn.Linear(in_features=151, out_features=k)
        self.soft=nn.Softmax(dim=1)

    ##########################################
    def forward(self, x):
        x = x.float()
        return self.soft(self.l2(torch.tanh(self.l1(x))))
    ...
    def fit(self, x, y, omega, tau, epochs, batch_size, lr):
        ...
        while epoch < epochs:
         ...
         ##########################################
            ...
            # # print(curve.shape)
            # plt.plot(omega.reshape([-1]), curve, label='norm pdf')
            # curve =scaled_cdf[0,:].detach().numpy()
            # plt.plot(omega.reshape([-1]), curve, label='scaled_cdf')
            # curve =skewed_norm[0,:].detach().numpy()
            # plt.plot(omega.reshape([-1]), curve, label='skewed_cdf')
            curve =yhat[0,:].detach().numpy()
            plt.plot((omega*2000).reshape([-1]), curve, label='Rhat')
            curve =y_batch[0,:].detach().numpy()
            plt.plot((omega*2000).reshape([-1]), curve, '--', label='R')
            plt.legend()
            plt.savefig("Samples_v1/Rhat"+str(epoch)+".png", dpi=300)
            plt.close()
            ...
            plt.plot((tau).reshape([-1]), curve, '--', label='E')
            plt.yscale('log')
            plt.legend()
...
omega=x['omega']
Loaded=np.load('/gpfs/jlse-fs0/users/kraghavan/Inverse/new_Data.npz')
E=Loaded['E'][0:10000,:]
R=Loaded['R'][0:10000,:]
...
R[R<1e-08] = 1e-08
rbfnet = Network(Kern, k=10)
rbfnet.to(device)
omega_fine = omega_fine/2000
# print("I moved the model to device")
_  =  rbfnet.fit(E, R, omega_fine, tau, 2000, 256, 0.001)
