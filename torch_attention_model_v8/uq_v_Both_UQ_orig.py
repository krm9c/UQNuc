...
class Network(nn.Module):
    ...
    def loss_func(self, Ehat, ENhat, Rhat,  E, EN, R, fac):
        ...
        loss_E = torch.mean( torch.mul((Ehat   -E).pow(2), (1/(fac[2]*fac[2])))) 
    ...
    def fit(self, trainloader, testloader_one, testloader_two,  omega, tau, epochs, batch_size, lr):
        ...
        while epoch < epochs:
            ...
            if epoch % 1 ==0:
                ...
                ## chi2_one.remove(max(chi2_one))
                ## chi2_one.remove(max(chi2_one))
                ## while sum(chi2_two)/len(chi2_two)>10:
                ##     print("removing once")
                ##     chi2_two.remove(max(chi2_two))
                ## print("The length after removing the larger onese is", chi2_two)
                ...
                for j in range(5):
                    ...
                    for x_batch, y_batch in testloader_one:
                        ...
                        for i in range(5):
                            x_hat, y_hat, _, yvar, xvar = self.forward(x_batch, 1e-4*pow(1,i))
                        ...
                        fig.tight_layout()
                        plt.savefig("results/sample_inverse/Rhat_One_wonoise_"+str(epoch)+'_'+str(j)+".png", dpi=300)
                        plt.close()
                        break
                    ...
                    for x_batch, y_batch in testloader_two:
                        ...
                        fig.tight_layout()
                        plt.savefig("results/sample_inverse/Rhat_Two_wonoise_"+str(epoch)+'_'+str(j)+".png", dpi=300)
...
xx = torch.from_numpy(E)
yy = torch.from_numpy(R)
trainset = MyDataset(xx, yy)
