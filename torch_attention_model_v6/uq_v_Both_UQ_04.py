...
class Network(nn.Module):
    ...
    def loss_func(self, Ehat, Rhat, Evar, Rvar, E, R, fac):
        ...
        loss_var= -torch.mean(Rvar-fac[2]-torch.mul(Rvar, torch.log(torch.div(Rvar, fac[2] )))) \
                  -torch.mean(Evar-fac[2]-torch.mul(Evar, torch.log(torch.div(Evar, fac[2] ))))
    ...
    ####################################################################################
            ...
            if epoch % 1 ==0:
                ...
                for x_batch, y_batch in testloader_one:
                    ...
                    break        
                fig.tight_layout()
                plt.savefig("sample_07/Rhat_One_"+str(epoch)+".png", dpi=300)
                plt.close()
                ...
                fig.tight_layout()
                plt.savefig("sample_07/Rhat_Two_"+str(epoch)+".png", dpi=300)
...
# Loaded=np.load('/grand/NuQMC/UncertainityQ/tyes
# heta_JLSE_Port/Inverse_new_Data.npz')
# # E=Loaded['E'][0:1000,:]
# R=Loaded['R'][0:10000,:]
...
rbfnet.load_state_dict(torch.load('one_two'))
rbfnet.to(device)
print("Everything is defined now")
rbfnet =  rbfnet.fit(trainloader, testloader_one, testloader_two, omega_fine, tau, 10, 128, 0.0001)
torch.save(rbfnet.state_dict(), 'one_two_v1')
